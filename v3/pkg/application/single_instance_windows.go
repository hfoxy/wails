...
func (a *App) sendMessage(hwnd w32.HWND, data string) {
...
func (a *App) setupSingleInstance(uniqueId string) {
	...
	if err != nil {
		if err == windows.ERROR_ALREADY_EXISTS {
			...
			if hwnd != 0 {
				...
				if err != nil {
					a.Logger.Warn("Failed to get working directory: %v", err)
					return
				}
				serialized, err := json.Marshal(data)
				if err != nil {
					a.Logger.Warn("Failed to marshal data: %v", err)
					return
				}

				a.sendMessage(hwnd, string(serialized))
		...
		a.createEventTargetWindow(className, windowName)
	}
}

func (a *App) createEventTargetWindow(className string, windowName string) w32.HWND {
	...
	wndProc := func(
		...
		if msg == w32.WM_COPYDATA {
			...
			if ldata.dwData == WMCOPYDATA_SINGLE_INSTANCE_DATA {
				...
				if err != nil {
					a.Logger.Warn("Failed to unmarshal data: %v", err)
					return w32.LRESULT(0)
				}

				// TODO: implement second instance buffer
